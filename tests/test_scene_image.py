...
def test_make_galaxy_image():
    ...
    # Bad file
    scene, stars = si.make_galaxy_image('foo/bar.txt', pos, filters[0])
    assert scene is None

    # Bad filter
    scene, stars = si.make_galaxy_image(file, pos, 'foobar')
    assert scene is None

    # Good filters
    for n, filt in enumerate(filters):
        scene, stars = si.make_galaxy_image(file, pos, filt)
        if n <= 5:
